...
/// Parse a generic table ID from an OpenFGA object string.
/// Format: `lakekeeper_generic_table:{warehouse_id}/{generic_table_id}`
/// Only returns the generic table ID if it belongs to the specified warehouse.
///
/// # Arguments
/// * `obj` - The OpenFGA object string to parse
/// * `warehouse_id` - The warehouse ID to filter by
///
/// # Returns
/// * `Ok(GenericTableId)` - The parsed generic table ID if it belongs to the specified warehouse
/// * `Err(OpenFGAError::InvalidEntity)` - If the object string is malformed or belongs to a different warehouse
pub(crate) fn parse_generic_table_from_openfga(
    obj: &str,
    warehouse_id: WarehouseId,
) -> OpenFGAResult<GenericTableId> {
    // Expected format: "lakekeeper_generic_table:{warehouse_uuid}/{generic_table_uuid}"
    let parts: Vec<&str> = obj.split(':').collect();
    if parts.len() != 2 || parts[0] != FgaType::GenericTable.to_string() {
        return Err(OpenFGAError::InvalidEntity(obj.to_string()));
    }
    ...
    // Filter to only include generic tables from the requested warehouse
    if parsed_warehouse_id != warehouse_id {
        return Err(OpenFGAError::InvalidEntity(format!(
            "Generic table {} belongs to different warehouse",
            obj
        )));
    }

    GenericTableId::from_str_or_bad_request(id_parts[1])
        .map_err(|e| OpenFGAError::InvalidEntity(format!("{}: {}", obj, e.message)))
}
...
mod test {
    ...
    #[test]
    fn test_parse_generic_table_from_openfga_valid() {
        let warehouse_id = WarehouseId::new_random();
        let generic_table_id = GenericTableId::new_random();
        let openfga_obj = (warehouse_id, generic_table_id).to_openfga();

        let parsed = parse_generic_table_from_openfga(&openfga_obj, warehouse_id).unwrap();
        assert_eq!(parsed, generic_table_id);
    }

    #[test]
    fn test_parse_generic_table_from_openfga_wrong_warehouse() {
        let warehouse_id = WarehouseId::new_random();
        let other_warehouse_id = WarehouseId::new_random();
        let generic_table_id = GenericTableId::new_random();
        let openfga_obj = (warehouse_id, generic_table_id).to_openfga();

        let result = parse_generic_table_from_openfga(&openfga_obj, other_warehouse_id);
        assert!(result.is_err());
        let err = result.unwrap_err();
        assert!(matches!(err, OpenFGAError::InvalidEntity(_)));
        assert!(err.to_string().contains("belongs to different warehouse"));
    ...
    #[test]
    fn test_parse_generic_table_from_openfga_wrong_type() {
        let warehouse_id = WarehouseId::new_random();
        let table_id = TableId::new_random();
        // Create a table object string instead of generic table
        let openfga_obj = (warehouse_id, table_id).to_openfga();

        let result = parse_generic_table_from_openfga(&openfga_obj, warehouse_id);
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), OpenFGAError::InvalidEntity(_)));
    }

    #[test]
    fn test_parse_generic_table_from_openfga_missing_slash() {
        let warehouse_id = WarehouseId::new_random();
        let malformed = format!("{}:{}", FgaType::GenericTable, warehouse_id);

        let result = parse_generic_table_from_openfga(&malformed, warehouse_id);
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), OpenFGAError::InvalidEntity(_)));
    }

    #[test]
    fn test_parse_generic_table_from_openfga_round_trip() {
        // Test that to_openfga and parse_generic_table_from_openfga are inverses
        let warehouse_id = WarehouseId::new_random();
        let generic_table_id = GenericTableId::new_random();

        let openfga_str = (warehouse_id, generic_table_id).to_openfga();
        let parsed = parse_generic_table_from_openfga(&openfga_str, warehouse_id).unwrap();
    ...
    fn test_parse_namespace_from_openfga_valid() {
        ...
        let parsed = NamespaceId::parse_from_openfga(&openfga_obj).unwrap();
    ...
    fn test_parse_namespace_from_openfga_wrong_type() {
        ...
        let result = NamespaceId::parse_from_openfga(&openfga_obj);
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
    ...
    fn test_parse_namespace_from_openfga_missing_colon() {
        ...
        let result = NamespaceId::parse_from_openfga(malformed);
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseOpenFgaEntityError::InvalidEntity(_)
        ));
    }
    ...
    fn test_parse_namespace_from_openfga_empty_string() {
        let result = NamespaceId::parse_from_openfga("");
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseOpenFgaEntityError::InvalidEntity(_)
    ...
    fn test_parse_namespace_from_openfga_round_trip() {
        // Test that to_openfga and NamespaceId::parse_from_openfga are inverses
        let namespace_id = NamespaceId::new_random();

        let openfga_str = namespace_id.to_openfga();
        let parsed = NamespaceId::parse_from_openfga(&openfga_str).unwrap();
